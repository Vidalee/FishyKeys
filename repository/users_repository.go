...
type UsersRepository interface {
	CreateUser(ctx context.Context, username string, hashedPassword string) error
...
func (r *usersRepository) CreateUser(ctx context.Context, username string, hashedPassword string) error {
	query := `
		INSERT INTO users (username, password, created_at, updated_at)
		VALUES ($1, $2, $3, $3)
		ON CONFLICT (username) DO NOTHING
	`
	_, err := r.pool.Exec(ctx, query, username, hashedPassword, time.Now().UTC())
	return err
}
